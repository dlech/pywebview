...
from winrt.microsoft.ui.interop import get_icon_id_from_icon, get_window_from_window_id
from winrt.microsoft.ui.windowing import (
    AppWindow,
...
from winrt.microsoft.web.webview2.core import (
    CoreWebView2,
    CoreWebView2Cookie,
...
from winrt.microsoft.windows.storage import ApplicationData
...
from winrt.windows.graphics import PointInt32, RectInt32, SizeInt32
...
class EdgeChrome:
    def __init__(self, form: Window, window: _Window, cache_dir: str):
        ...
        self.webview = WebView2._from(Grid._from(form.content).find_name("webview"))
        self.form = form

        self.js_result_semaphore = Semaphore(0)
        self.webview.add_core_web_view2_initialized(self.on_webview_ready)
        ...
        def env_callback(
            ...
            ctrl_options = env.create_core_web_view2_controller_options()
            ctrl_options.is_in_private_mode_enabled = _state["private_mode"]

            ensure_op = (
                self.webview.ensure_core_web_view2_with_environment_and_options_async(
                    env, ctrl_options
                )
            )

            def ensure_callback(op: IAsyncAction, status: AsyncStatus):
                if status == AsyncStatus.ERROR:
                    logger.error(
            ...
            ensure_op.completed = ensure_callback

        env_op.completed = env_callback
    ...
    def clear_cookies(self):
        self.webview.core_web_view2.cookie_manager.delete_all_cookies()
    ...
    def get_cookies(self, semaphore: Semaphore, cookies: list[SimpleCookie]):
        ...
        op = self.webview.core_web_view2.cookie_manager.get_cookies_async(self.url)
    ...
    def load_html(self, content: str, _: str):
        ...
        if self.webview.core_web_view2:
            self.webview.core_web_view2.navigate_to_string(self.html)
        else:
            self.webview.ensure_core_web_view2_async()
    ...
    def on_webview_ready(
        ...
        sender.core_web_view2.add_source_changed(self.on_source_changed)
        sender.core_web_view2.add_new_window_requested(self.on_new_window_request)

        if _state["ssl"]:
            sender.core_web_view2.add_server_certificate_error_detected(
                self.on_certificate_error
            )

        sender.core_web_view2.add_download_starting(self.on_download_starting)

        settings = sender.core_web_view2.settings
        ...
        if _state["private_mode"]:
            ...
            sender.core_web_view2.cookie_manager.delete_all_cookies()
        ...
        if _state["debug"] and webview_settings["OPEN_DEVTOOLS_IN_DEBUG"]:
            sender.core_web_view2.open_dev_tools_window()
...
class BrowserView:
    ...
    class BrowserForm:
        def __init__(self, window: _Window, cache_dir: str):
            ...
            self.window.content = XamlReader.load(_WINDOW_XAML)
            self.window.app_window.resize(
                SizeInt32(window.initial_width, window.initial_height)
            )
            ...
            if window.initial_x is not None and window.initial_y is not None:
                self.window.app_window.move(
                    PointInt32(window.initial_x, window.initial_y)
                )
            elif window.screen:
                # TODO: make sure this works
                area = DisplayArea.get_from_display_id(window.screen.frame)
                x = (area.work_area.width - self.window.app_window.size.width) // 2
                y = (area.work_area.height - self.window.app_window.size.height) // 2
                self.window.app_window.move(PointInt32(x, y))
            else:
            ...
            self.overlapped_presenter = OverlappedPresenter._from(
                self.window.app_window.presenter
            )
        ...
        def on_changed(self, sender: AppWindow, args: AppWindowChangedEventArgs):
            if self.overlapped_presenter.state != self.old_state:
                ...
                pos = self.window.app_window.position
                self.pywebview_window.events.moved.set(pos.x, pos.y)
        ...
        def set_window_menu(self, menu_list: Iterable[Menu]):
            ...
            top_level_menu = MenuBar._from(
                Grid._from(self.window.content).find_name("menu")
            )
        ...
        @invoke_on_ui_thread
        def get_size(self):
            size = self.window.app_window.size
            return size.width, size.height

        @invoke_on_ui_thread
        def resize(self, width: int, height: int, fix_point: FixPoint):
            x = self.window.app_window.position.x
            y = self.window.app_window.position.y
            ...
            self.window.app_window.move_and_resize(RectInt32(x, y, width, height))
